...
(function() {
    chrome.storage.local.get(['stealth'], function(result) {
        ...
        document.addEventListener("keydown", (e) => {
            // CHATBOT DISABLED - Alt+C functionality removed
        ...
        async function init() {
            // CHATBOT DISABLED - Initialization disabled
            // To re-enable chatbot, uncomment the code below:
            /*
            try {
            ...
            */
            console.log('[macneopassfree] Chatbot feature is currently disabled');
        }
        ...
        chrome.storage.onChanged.addListener((changes, namespace) => {
            if (namespace === 'local') {
                // Clear error state when backend-related settings change
                if (changes.useCustomAPI || changes.aiProvider || changes.customEndpoint || changes.customAPIKey || changes.customModelName) {
                    clearErrorState();
                    console.log("Backend settings changed, cleared chat error state");
                }
